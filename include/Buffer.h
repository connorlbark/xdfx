...
public:
    ...
    void mult_inplace(Buffer<F> other) {
        for (int i = 0; i < this->length(); i++) {
            this[i] = other[i] * this[i];
        }
    }

    void wet_dry(Buffer<F> wet, float wet_pct) {
        float dry_pct = 1.f - wet_pct;

        for (int i = 0; i < this->length(); i++) {
            this[i] = dry_pct * this[i] + wet_pct * wet[i];
        }
    }
};


#endif //STDFX_BUFFER_H
